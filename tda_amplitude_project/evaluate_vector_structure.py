...
def main():
    ...
    parser.add_argument("--amplitude_metric", type=str, default="")
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    metric_suffix = f" ({args.amplitude_metric})" if args.amplitude_metric != "" else ""
    ...
    plot_single_embedding(
        tsne_true, labels,
        f"t-SNE True {metric_suffix}",
        out_dir / "tsne_true.png"
    )
    plot_single_embedding(
        tsne_pred, labels,
        f"t-SNE Predicted {metric_suffix}",
        out_dir / "tsne_pred.png"
    )
    plot_overlay(
        tsne_true, tsne_pred, labels,
        f"t-SNE Overlay: True (o) vs Predicted (x){metric_suffix}",
    ...
    plot_single_embedding(
        umap_true, labels,
        f"UMAP True{metric_suffix}",
        out_dir / "umap_true.png"
    )
    plot_single_embedding(
        umap_pred, labels,
        f"UMAP Predicted{metric_suffix}",
        out_dir / "umap_pred.png"
    )
    plot_overlay(
        umap_true, umap_pred, labels,
        f"UMAP Overlay: True (o) vs Predicted (x){metric_suffix}",
